...
const CrmDashboardPage = () => {
    ...
    if (loading) return <div className="text-sm text-gray-500 mt-10 text-center">Cargando resumen...</div>;

    return (
        <div className="flex flex-col gap-6">
            <div>
                <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Resumen First Class</h1>
                <p className="text-sm text-gray-500 mt-0.5">Embudo de conversión y rendimiento de marketing.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                    <h2 className="font-bold text-gray-900 mb-4">Embudo de conversión</h2>
                    ...
                    {totalProspects === 0 && <p className="text-xs text-gray-400 mt-4 text-center">Aún no hay prospectos registrados.</p>}
                </div>

                {/* ROI por campaña */}
                <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
                    <h2 className="font-bold text-gray-900 mb-4 flex items-center gap-2"><Megaphone size={16} className="text-yellow-500" /> Retorno por campaña</h2>
                    {roi.length === 0 ? (
                        <p className="text-xs text-gray-400 text-center py-6">Sin campañas registradas.</p>
                    ) : (
                        <div className="overflow-x-auto -mx-1">
                            <table className="w-full text-left text-sm">
                                ...
                                <tbody className="divide-y divide-gray-100">
                                    {roi.map((r, i) => (
                                        <tr key={i}>
                                            <td className="py-2 pr-2 font-semibold text-gray-800 truncate max-w-[120px]">{r.campaign_name}</td>
                                            <td className="py-2 px-2 text-center text-gray-600">{r.leads}</td>
                                            <td className="py-2 px-2 text-center font-bold text-green-600">{r.enrolled}</td>
                                            <td className="py-2 pl-2 text-right text-gray-600">{r.enrolled > 0 ? money(r.cost_per_enrolled) : '—'}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
...
const Metric = ({ icon: Icon, label, value, accent }) => (
    <div className={`rounded-xl border p-4 shadow-sm ${accent ? 'bg-green-50 border-green-100' : 'bg-white border-gray-200'}`}>
        <Icon size={18} className={accent ? 'text-green-600' : 'text-gray-400'} />
        <p className={`text-2xl font-bold mt-2 ${accent ? 'text-green-700' : 'text-gray-900'}`}>{value}</p>
        <p className="text-[11px] text-gray-400 uppercase tracking-wide">{label}</p>
    </div>
);

const QuickLink = ({ to, icon: Icon, title, subtitle }) => (
    <Link to={to} className="group bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:border-yellow-300 hover:shadow-md transition-all flex items-center gap-4">
        <div className="p-3 bg-yellow-50 text-yellow-600 rounded-lg"><Icon size={22} /></div>
